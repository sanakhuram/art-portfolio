...
export function getCategoryData(category: Category): CategoryInfo {
  ...
  metadata.paintings = metadata.paintings.map((p) => ({
    ...
    extraImages: p.extraImages
      ? p.extraImages.map((img: string) => `/images/${FOLDER_MAP[category]}/${img}`)
      : undefined,

    video: p.video ? `/images/${FOLDER_MAP[category]}/${p.video}` : undefined,
  })) as Painting[];

  return metadata;
}

export function getPainting(category: Category, slug: string): Painting | undefined {
